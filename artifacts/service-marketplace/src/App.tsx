...
const categories = [
  { title: 'صيانة منزلية', detail: 'سباكة، كهرباء وأكثر', icon: Wrench, color: 'bg-[#e8dfc4]' },
...
function Logo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className="group inline-flex items-center gap-2.5" data-testid="link-logo">
      ...
      {!compact && <span className="font-display text-xl font-bold tracking-tight text-primary">مَعُون</span>}
    </Link>
  );
}
    ...
    <div className="app-noise min-h-[100dvh] bg-background">
      <div className="mx-auto flex min-h-[100dvh] max-w-[1440px]">
        <aside className={`fixed inset-y-0 right-0 z-50 w-[280px] border-l border-sidebar-border bg-sidebar p-6 text-sidebar-foreground shadow-lg transition-transform md:sticky md:top-0 md:flex md:h-[100dvh] md:w-[240px] md:translate-x-0 md:flex-col md:shadow-none ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="flex items-center justify-between md:justify-center"><Logo compact /><span className="font-display text-lg font-bold text-sidebar-foreground md:hidden">مَعُون</span><IconButton label="إغلاق القائمة" onClick={() => setMenuOpen(false)}><X size={19} /></IconButton></div>
...
function WelcomePage() {
  return (
    <div className="app-noise min-h-[100dvh] overflow-hidden bg-background">
      ...
      <main>
        <section className="mx-auto grid max-w-6xl items-center gap-12 px-5 pb-16 pt-8 md:grid-cols-[1.05fr_.95fr] md:px-8 md:pb-24 md:pt-16">
          <div className="float-in order-2 md:order-1">
            ...
            <p className="mt-6 max-w-lg text-lg leading-9 text-muted-foreground">مَعُون يقرّبك من أشخاص موثوقين في حيك. اطلب ما تحتاج، أو شارك خبرتك مع من حولك.</p>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative w-[min(88%,390px)] rotate-2 rounded-[2rem] border border-primary/10 bg-card p-4 shadow-lg">
                <div className="rounded-[1.4rem] bg-primary p-5 text-primary-foreground">
                  <div className="flex items-start justify-between"><div><p className="text-xs text-primary-foreground/65">صباح الخير، يا نورة</p><p className="mt-1 font-display text-2xl font-bold">كيف نعاونك اليوم؟</p></div><span className="rounded-xl bg-primary-foreground/10 p-2"><Sparkles size={18} className="text-accent" /></span></div>
                  <div className="mt-6 flex items-center gap-2 rounded-2xl bg-primary-foreground/10 px-4 py-3 text-sm text-primary-foreground/70"><Search size={17} /><span>ابحث عن خدمة قريبة...</span></div>
                </div>
        ...
        <section className="bg-primary py-14 text-primary-foreground">
          <div className="mx-auto grid max-w-6xl gap-10 px-5 md:grid-cols-3 md:px-8">
            {[{ n: '١', title: 'اختر ما يناسبك', text: 'تصفّح الخدمات أو عرّفنا بخبرتك.' }, { n: '٢', title: 'تواصل بوضوح', text: 'اطّلع على الملف والخبرة قبل أي خطوة.' }, { n: '٣', title: 'ابدأ على راحتك', text: 'لا التزامات مخفية. القرار لك دائماً.' }].map((item) => <div key={item.n} className="flex gap-4 border-b border-primary-foreground/15 pb-7 last:border-0 md:border-b-0 md:border-r md:pb-0 md:pr-8 md:last:border-0"><span className="font-display text-4xl text-accent">{item.n}</span><div><h2 className="font-display text-lg font-bold">{item.title}</h2><p className="mt-2 text-sm leading-7 text-primary-foreground/65">{item.text}</p></div></div>)}
          </div>
        </section>
      ...
      <footer className="border-t border-border px-5 py-6 text-center text-xs text-muted-foreground">مَعُون — نكبر حين نعاون بعضنا</footer>
    </div>
  );
}
    ...
    <div className="app-noise min-h-[100dvh] bg-primary px-5 py-5 text-primary-foreground md:px-10">
      ...
      <main className="mx-auto grid max-w-5xl items-center gap-10 py-12 md:grid-cols-[.85fr_1.15fr] md:py-20">
        ...
        <div className="rounded-[2rem] bg-card p-5 text-foreground shadow-lg sm:p-8">
          ...
          <p className="mt-5 text-center text-xs leading-6 text-muted-foreground">بالاستمرار، أنت توافق على تجربة مَعُون المحلية. <br />لا يوجد اتصال بخدمة خارجية في هذا العرض.</p>
        </div>
      </main>
    </div>
  );
...
function BeneficiaryHome({ profile }: { profile: Profile }) {
  ...
  return (
    <AppShell role="beneficiary" profile={profile}>
      <section className="relative overflow-hidden rounded-[2rem] bg-primary px-6 py-8 text-primary-foreground md:px-10 md:py-11"><div className="relative z-10 max-w-xl"><p className="text-sm text-primary-foreground/65">صباح الخير، {profile.name === 'زائر جديد' ? 'يا صديق' : profile.name.split(' ')[0]}</p><h1 className="mt-2 font-display text-3xl font-bold leading-tight md:text-4xl">وش نعاونك فيه اليوم؟</h1><p className="mt-3 text-sm leading-7 text-primary-foreground/65">ابحث عن شخص قريب يفهم احتياجك.</p><div className="mt-7 flex items-center gap-3 rounded-2xl bg-primary-foreground/10 px-4 py-1"><Search size={19} className="shrink-0 text-accent" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="مثال: ترتيب منزل، كهرباء..." className="h-12 min-w-0 flex-1 bg-transparent text-sm text-primary-foreground outline-none placeholder:text-primary-foreground/45" data-testid="input-search-services" /><SlidersHorizontal size={18} className="text-primary-foreground/60" /></div></div><div className="absolute -left-10 -top-16 size-64 rounded-full border-[22px] border-primary-foreground/5" /><div className="absolute -bottom-28 right-1/2 size-72 rounded-full border-[30px] border-accent/10" /></section>
