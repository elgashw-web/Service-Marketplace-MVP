...
const categories = [
  { title: 'خدمات منزلية', detail: 'سباكة، كهرباء وصيانة', icon: Wrench, color: 'bg-[#e8dfc4]' },
  { title: 'رعاية ومرافقة', detail: 'مساعدة في حياتك اليومية', icon: HandHeart, color: 'bg-[#dfe8d6]' },
  { title: 'نقل ومساعدة', detail: 'نساعدك في التنقل', icon: CarFront, color: 'bg-[#e2e4d4]' },
  { title: 'دروس ومهارات', detail: 'تعلّم من أصحاب الخبرة', icon: BookOpen, color: 'bg-[#eaded3]' },
];
    ...
    <header className="flex items-center justify-between py-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href="/profile" className="hidden items-center gap-2 rounded-2xl p-1.5 pr-3 transition-colors hover:bg-secondary sm:flex" data-testid="link-profile-top">
          <span className="text-right"><span className="block text-xs text-muted-foreground">مرحباً بك</span><span className="block text-sm font-bold text-primary">{profile.name}</span></span>
...
function BottomNav({ role }: { role: Role }) {
  const items = role === 'provider'
    ? [{ href: '/provider/home', label: 'الرئيسية', icon: Home }, { href: '/profile', label: 'الملف الشخصي', icon: UserCircle }]
    : [{ href: '/beneficiary/home', label: 'الرئيسية', icon: Home }, { href: '/profile', label: 'الملف الشخصي', icon: UserCircle }];
  return (
      ...
      <main className="mx-auto max-w-5xl px-5 md:px-8">
        <section className="landing-hero pt-8 md:pt-14">
          <div className="landing-hero-copy float-in">
            <p className="landing-greeting text-sm font-bold text-[#9c6d32]">مرحباً بك</p>
            <h1 className="landing-title font-display mt-3 font-bold tracking-tight text-primary">كيف يمكننا مساعدتك اليوم؟</h1>
          ...
          <div className="landing-service-section">
            <h2 className="landing-section-title font-display font-bold text-primary">اختر الخدمة التي تحتاجها</h2>
        ...
        <section className="landing-trusted flex items-center gap-3 border-t border-border">
          ...
          <div><strong className="block text-sm font-bold text-primary">أشخاص موثوقون بالقرب منك</strong><small className="text-xs text-muted-foreground">بخطوات بسيطة وآمنة</small></div>
        </section>
        <section className="landing-how border-t border-border" id="how">
          ...
          <div className="landing-steps mt-8">
            {[{ n: '١', title: 'انضم إلى مجتمع يقدّم المساعدة بصدق', text: '' }, { n: '٢', title: 'اختر ما يناسبك', text: 'تصفح الخدمات أو شاركنا خبرتك.' }, { n: '٣', title: 'تواصل بوضوح', text: 'اطّلع على الملف والخبرة قبل الاتفاق.' }, { n: '٤', title: 'ابدأ بثقة', text: 'لا توجد التزامات مخفية. القرار لك دائماً.' }].map((item) => <div key={item.n} className="landing-step flex gap-4"><span className="font-display text-3xl font-bold text-[#9c6d32]">{item.n}</span><div><h3 className="font-display text-base font-bold text-primary">{item.title}</h3>{item.text && <p className="mt-1.5 text-sm leading-7 text-muted-foreground">{item.text}</p>}</div></div>)}
          </div>
        </section>
      </main>
    ...
    <div className="app-noise min-h-[100dvh] bg-primary px-5 py-5 text-primary-foreground md:px-10">
      ...
      <main className="mx-auto grid max-w-5xl items-center gap-10 py-12 md:grid-cols-[.85fr_1.15fr] md:py-20">
        ...
        <div className="rounded-[2rem] bg-card p-5 text-foreground shadow-lg sm:p-8">
           <div className="mb-7 md:hidden"><span className="text-xs font-bold text-[#9c6d32]">جلسة تجريبية محلية</span><h1 className="mt-2 font-display text-3xl font-bold text-primary">مرحباً بك في مُعُون</h1></div>
...
function ProviderHome({ profile }: { profile: Profile }) {
  return (
    <AppShell role="provider" profile={profile}>
       <section className="relative overflow-hidden rounded-[2rem] border border-[#d5dfcb] bg-[#e8efdf] px-6 py-8 md:px-10 md:py-10"><div className="relative z-10"><p className="text-sm text-primary/60">مساحتك المهنية</p><h1 className="mt-2 max-w-lg font-display text-3xl font-bold leading-tight text-primary md:text-4xl">دع الآخرين يعرفون كيف يمكنك مساعدتهم.</h1><p className="mt-3 max-w-md text-sm leading-7 text-primary/70">ملف واضح وبسيط يكفي لبدء التعارف.</p><Button href="/profile" variant="primary" className="mt-6" testId="link-provider-edit-profile">تعديل الملف <Pencil size={16} /></Button></div><div className="absolute -left-9 -top-10 size-52 rounded-full bg-accent/20" /><BriefcaseBusiness className="absolute bottom-7 left-9 text-primary/10" size={110} /></section>
